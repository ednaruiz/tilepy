...
filename = (
    "/Users/mseglar/Documents/CurrentPhD/HESS/GW/going-the-distance-o2-skymaps/bayestar/"
    + name[j]
)
...
AuxObservationTime1, AuxObservationTime2 = np.genfromtxt(
    Timefilename, usecols=(0, 1), unpack=True, dtype="str"
)
AuxObservationTime = (
    AuxObservationTime1[j + 250 * r] + " " + AuxObservationTime2[j + 250 * r]
)
print(AuxObservationTime)
ObservationTime0 = datetime.datetime.strptime(AuxObservationTime, "%Y-%m-%d %H:%M:%S")

start = time.time()
SuggestedPointings, cat, area = PGalonFoV_PixRegion(filename, ObservationTime0)
end = time.time()
...
outfilename = "./ResultsPixRegion/GWInjection" + str(j) + "Round_" + str(r) + ".txt"
