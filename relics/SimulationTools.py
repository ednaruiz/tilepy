    ...
    def from_fitsfile(cls, filepath, absorption):
        ...
        for interval in range(0, len(time_interval)):
            ...
            table_model = TableModel(
                energy=energy_interval, values=newflux, values_scale="log"
            )
...
def DynamicPointingSimulation(
    ...
    for k in range(0, len(LC_intTimes)):
        ...
        sky_model = SkyModel(
            ...
            spectral_model=grb.spectral_model[len(mask) - 1],
        )
        npred, c, b, e = PointingSimulation(
            sky_model, axis, geom, pointing, livetime1, offset, irfs
        )
...
def CubeSimulationMultiObservations(
    InputListrun,
    InputListMergerID,
    pointing,
    nP,
    ...
    spatial_model = SkyGaussian(
        lon_0=Source.ra.deg * u.deg, lat_0=Source.dec.deg * u.deg, sigma="0.1 deg"
    )
    ...
    energy_axis = MapAxis.from_edges(
        np.logspace(-2.0, 10, 10), unit="TeV", name="energy", interp="log"
    )
    ...
    for i in range(0, len(livetimes)):
        ...
        if efLivetimes[i] > 0:
            geom = WcsGeom.create(
                skydir=(RAP[counterPointings], DecP[counterPointings]),
                binsz=0.04,
                width=(10, 10),
                coordsys="CEL",
                axes=[axis],
            ...
            pointing = SkyCoord(
                RAP[counterPointings], DecP[counterPointings], unit="deg", frame="fk5"
            )
            Cube4DMap, Background4DMap, Exposure4DMap = DynamicPointingSimulation(
                slew,
                slew + timestep,
                TimeAxis,
                efLivetimes[i],
...
def CubeSimulationMultiObservations_dynamicSpectrum(
    InputListrun,
    InputListMergerID,
    pointing,
    nP,
    ...
    time_axis = MapAxis.from_edges(
        np.arange(len(pointing) + 1), unit="second", name="time"
    )
    print(time_axis)
    print(energy_axis)
    geom3D = WcsGeom.create(
    ...
    for i in range(0, len(pointing)):
        ...
        sky_model = SkyModel(
            spatial_model=spatial_model, spectral_model=grb.spectral_model[10]
        )
        npred, counts, background, exposure = PointingSimulation(
            sky_model, energy_axis, geom, pointing, 15 * u.second, offset, irfs
        )
    ...
    maps["background"].write(
        str(fullpath / "background_multipleObs.fits"), overwrite=True
    )
    maps["exposure"].write(str(fullpath / "exposure_multipleObs.fits"), overwrite=True)


def CubeSimulationSingleObservation_dynamicSpectrum(
    InputListrun,
    InputListMergerID,
    nP,
    Source,
    ...
    PointingsCoord = SkyCoord(
        Pointings["RA(deg)"], Pointings["DEC(deg)"], frame="fk5", unit=(u.deg, u.deg)
    )
    ...
    time_axis = MapAxis.from_edges(
        SimulationInterval, unit="second", name="time", interp="log"
    )
    geom3D = WcsGeom.create(
        skydir=(RAP[nP], DecP[nP]),
        binsz=0.04,
        width=(16, 14),
        coordsys="CEL",
        axes=[energy_axis],
    ...
    for i in range(0, len(SimulationInterval) - 1):
        ...
        sky_model = SkyModel(
            spatial_model=spatial_model,
            spectral_model=grb.spectral_model[FirstSpectralModel + i],
        )
        npred, counts, background, exposure = PointingSimulation(
            sky_model, axis, geom, pointing, efLivetimes * u.second, offset, irfs
        )
    ...
    # write maps
    maps["counts"].write(
        str(fullpath / "counts_singleObs_dynamic.fits"), overwrite=True
    )
    maps["background"].write(
        str(fullpath / "background_singleObs_dynamic.fits"), overwrite=True
    )
    maps["exposure"].write(
        str(fullpath / "exposure_singleObs_dynamic.fits"), overwrite=True
    )
...
def CubeSimulationSingleObservation_singleSpectrum(
    InputListrun,
    InputListMergerID,
    nP,
    Source,
    ...
    spatial_model = SkyGaussian(
        lon_0=Source.ra.deg * u.deg, lat_0=Source.dec.deg * u.deg, sigma="0.2 deg"
    )
    ...
    # Cube Definition
    axis = MapAxis.from_edges(
        np.logspace(-3.0, 1.0, 20), unit="TeV", name="energy", interp="log"
    )
    ...
    npred, counts, background, exposure = PointingSimulation(
        sky_model, axis, geom3D, pointing, livetime, offset, irfs
    )
    Cube3DMap.data = counts
    Background3DMap.data = background.data
    Exposure3DMap.data = exposure.data
    ...
    maps["background"].write(
        str(fullpath / "background_singleObs.fits"), overwrite=True
    )
