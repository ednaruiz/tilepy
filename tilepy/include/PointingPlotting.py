...
def PlotPointings(
    ...
    if doPlot:
        ...
        hp.mollview(
            ...
            rot=[180, 0],
            coord="C",
            title="GW prob map (Ecliptic) + %s %g  %s/%s/%s %s:%s:%s UTC"
            % (
                name,
                Totalprob * 100,
                time[0].day,
...
def PlotPointingsTogether(
    prob, time, targetCoord1, n1, targetCoord2, n2, nside, FOV, doPlot=True
):
...
def PointingPlottingGW_ZenithSteps(
    filename, name, dirName, FOV, InputTimeObs, ObsArray
):
    print()
    print("-------------------   PLOTTING SCHEDULE   --------------------")
    print()
...
def PlotPointings_Pretty(filename, name, PointingsFile1, dirName, obspar):
    ...
    time = []
    try:
        time1, time2, ra, dec, pgw, pgal, Round, nametel = np.genfromtxt(
            tpointingFile,
        ...
        try:
            time1, time2, ra, dec, pgw, Round, nametel = np.genfromtxt(
                tpointingFile,
            ...
            try:
                ...
                time1, time2, ra, dec, pgw, Round = np.genfromtxt(
                    tpointingFile,
                    usecols=(0, 1, 2, 3, 4, 5),
                    skip_header=1,
                    delimiter=" ",
    ...
    for i in range(0, len(ra)):
        ...
        c = Circle(
            ...
            edgecolor=COLORS,
            facecolor="None",
            transform=ax_inset.get_transform("fk5"),
            alpha=1,
        )
