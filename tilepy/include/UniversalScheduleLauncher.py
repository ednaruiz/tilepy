...
from .RankingObservationTimes import RankingTimes_2D
    ...
    if len(SuggestedPointings) != 0:
        ...
        for j in range(len(obsparameters)):
            ...
            SuggestedPointings_1 = SuggestedPointings[
                SuggestedPointings["ObsName"] == obspar1.name
            ]
            ...
            if len(SuggestedPointings_1) != 0:
                ...
                RankingTimes_2D(
                    ...
                    prob,
                    obsparameters[j],
                    obsparameters[j].alertType,
                    dirName,
                    "%s/SuggestedPointings_GWOptimisation_%s.txt"
                    % (dirName, obsparameters[j].name),
                    obsparameters[j].name,
                )
                PointingPlotting(
                    prob,
                    obsparameters[j],
