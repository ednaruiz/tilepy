...
iers_file = os.path.join(
    os.path.abspath(os.path.dirname(__file__)), "../dataset/finals2000A.all"
)
...
class Tools:
    ...
    def SunAlt(cls, obsTime, obspar):
        sun = get_sun(Time(obsTime, scale="utc")).transform_to(
    ...
    def TrustingDarknessSun(cls, obsTime, obspar):
        ...
        while Tools.IsDarkness(DarkObsTime, obspar) == False and (
            (
    ...
    def TrustingGreynessSun(cls, obsTime, obspar):
        GreyObsTime = obsTime
        referencetime = obsTime
        while Tools.IsGreyness(GreyObsTime, obspar) == False and (
            (
...
def GetGWMap_Flat(URL):
    ...
    if "multiorder." in filename:
        ...
        print(
            "The GW map is not in multiorder format, we will try the .fits.gz format, you are welcome"
        )
   ...
   :return tprob : array of p-values as a function of sky position
    ...
    if fitsfile[1].header["TFIELDS"] <= 2:
        ...
        print(
            "Event has triggered ",
            tdetectors,
            " => distance = {0:.2f}".format(tdistmean),
            " +- {0:.2f}".format(tdisterr),
            " Mpc",
        )
...
def Intersect2D(filename, intersectionThres, obspar):
    ...
    for i in range(len(theta)):
        ...
        sinb = np.sin(decngp) * np.sin(dec) + np.cos(decngp) * np.cos(dec) * np.cos(
            ra - rangp
        )
    ...
    if 1 >= totprob >= intersectionThres / 100:
        ...
        return (
            ...
            totprob,
            ver,
        )  # a big part of the GW falls behind the galactic plane, so we need to use the 3D method
        ...
        return (
            ...
            totprob,
            ver,
        )  # the percentage of GW behind the galactic plane is still low enough to apply the 2D method
...
def CorrelateGalaxies_LVC(
    prob, distmu, distsigma, distnorm, cat, Info3D_available, MinimumProbCutForCatalogue
):
    """
    ...
    if Info3D_available:
        dp_dV = (
            prob[ipix]
            * distnorm[ipix]
            * norm(distmu[ipix], distsigma[ipix]).pdf(dist)
            / pixarea
        )
...
def CorrelateGalaxies_LVC_SteMass(
    prob,
    distmu,
    distsigma,
    distmean,
    ...
    if Info3D_available:
        dp_dV_pos = (
            prob[NewIpix]
            * distnorm[NewIpix]
            * norm(distmu[NewIpix], distsigma[NewIpix]).pdf(dist)
            / pixarea
        )
...
def FulfillsRequirement_MinProb(thisGals_aux, maxz):
    ...
    for minz_aux in range(maxz, 5, -5):
        ...
        if len(tmpGals[tmpmask]) > 0:
            ...
            print(
                "{0} galaxies visible with zen < {1} deg - maximum p-value {2:0.3f}".format(
                    len(tmpGals[tmpmask]), minz_aux, cur_maxp
                )
            )
            print(
                "Maximum probability of {0:0.3f} of global maximum of {1:3f}".format(
...
def ComputeProbBCFOVSimple(
    ...
    if doPlot:
        ...
        hp.mollview(
            prob,
            title="GW prob map (Ecliptic)   %s/%s/%s %s:%s UTC"
...
def ComputeProbBCFOV(
    ...
    if doPlot:
        ...
        hp.visufunc.projplot(
            RandomCoordmin_radec.ra,
            RandomCoordmin_radec.dec,
            "y.",
            lonlat=True,
            coord="C",
            marker=".",
...
def SubstractPointings(
    tpointingFile, galaxies, talreadysumipixarray, tsum_dP_dV, FOV, prob, nside
):
...
def ModifyCataloguePIX(
    ...
    cat_pix = Table(
        [pix_ra, pix_dec, dp_Pix_Fov], names=("PIXRA", "PIXDEC", "PIXFOVPROB")
    )
    ...
    for l in range(0, len(cat_pix)):
        dp_dV_FOV.append(
            ComputePGalinFOV(prob, cat, cat_pix[l], FOV, totaldPdV, nside, UsePix=True)
        )
...
def LoadGalaxies_GladeCTASimu(tgalFile):
    ...
    name, dist, z, ra, dec, flag = np.genfromtxt(
        tgalFile, usecols=(0, 1, 2, 3, 4, 5), skip_header=3, unpack=True, dtype="str"
    )  # ra, dec in degrees
    ra = ra.astype(float)
    ...
    tcat = Table(
        [name, ra, dec, dist, z, flag],
        names=("Galaxy", "RAJ2000", "DEJ2000", "Dist", "z", "flag"),
    )
    # print(tcat)
    return tcat
...
def LoadGalaxiesSimulation(tgalFile):
    ...
    dist, z, ra, dec = np.genfromtxt(
        tgalFile, usecols=(1, 2, 3, 4), skip_header=3, unpack=True
    )  # ra, dec in degrees
...
def PointingFileReadCTA(pointingFile):
    time1, time2, RA, Dec, Observatory, ZenIni, ZenEnd, Duration = np.genfromtxt(
        pointingFile,
    ...
    OutputTable = Table(
        [time, RA, Dec, Observatory, ZenIni, ZenEnd, Duration],
        names=[
...
def TableImportCTA_simple(inputFileName):
    run, MergerID, RA, Dec, distance, z, theta, ndet, SNR, A90, A50 = np.genfromtxt(
        inputFileName,
    ...
    OutputTable = Table(
        [run, MergerID, RA, Dec, distance, z, theta, ndet, SNR, A90, A50],
        names=(
            "run",
            "MergerID",
            "RA",
...
def TableImportCTA_Glade(tgalFile):
    ...
    A50 = A50.astype(float)
    OutputTable = Table(
        [run, gal, MergerID, RA, Dec, distance, z, theta, ndet, SNR, A90, A50],
        names=(
            "run",
            "Galaxy",
            "MergerID",
...
def TableImportCTA_TimeNoZenith(ttimeFile):
    run, MergerID, time1, time2, Observatory = np.genfromtxt(
        ttimeFile, usecols=(0, 1, 2, 3, 4), skip_header=1, unpack=True, dtype="str"
    )
    time = []
    for i in range(len(time1)):
        time.append((time1[i] + " " + time2[i]).split('"')[1])
    OutputTable = Table(
        ...
        names=["run", "MergerID", "Time", "Observatory"],
    )
...
def TableImportCTA_Petrov(ttimeFile):
    ID, time1, time2 = np.genfromtxt(
        ttimeFile, usecols=(0, 1, 2), skip_header=1, unpack=True, dtype="str"
    )
    time = []
    for i in range(len(time1)):
        time.append((time1[i] + " " + time2[i]).split('"')[1])
...
def TableImportCTA_SetOfTimes(ttimeFile):
    trial, run, MergerID, time1, time2, Observatory = np.genfromtxt(
        ttimeFile, usecols=(0, 1, 2, 3, 4, 5), skip_header=1, unpack=True, dtype="str"
    )
    time = []
    for i in range(len(time1)):
        time.append((time1[i] + " " + time2[i]).split('"')[1])
    OutputTable = Table(
        [run, MergerID, trial, time, Observatory],
...
def TableImportCTA_Time(ttimeFile):
    run, MergerID, time1, time2, MeanAlt, Observatory = np.genfromtxt(
        ttimeFile, usecols=(0, 1, 2, 3, 4, 5), skip_header=1, unpack=True, dtype="str"
    )
    ...
    for i in range(len(time1)):
        time.append((time1[i] + " " + time2[i]).split('"')[1])
    OutputTable = Table(
        [run, MergerID, time, MeanAlt, Observatory],
...
def TableImportCTA_Obs(tobsFile):
    pointing, tI, tF, interval = np.genfromtxt(
        tobsFile, usecols=(0, 1, 2, 3), skip_header=3, unpack=True, dtype="int"
    )
    OutputTable = Table(
        [pointing, tI, tF, interval], names=["pointingNumber", "tI", "tF", "Interval"]
    )
    return OutputTable


def TableImportCTA_LS(tgalFile):
    eventid, RA, Dec, distance = np.genfromtxt(
        tgalFile, usecols=(0, 3, 4, 8), skip_header=1, unpack=True, dtype="str"
    )
    RA = RA.astype(float)
    Dec = Dec.astype(float)
    distance = distance.astype(float) / 1000  # to Mpc!
    OutputTable = Table(
        [eventid, RA, Dec, distance], names=("MergerID", "RA", "Dec", "Distance")
    )
    return OutputTable
...
def ProduceSummaryFileOld(
    ...
    filepath = (
        ...
        + str(InputList["run"][j])
        + "_"
        + str(InputList["MergerID"][j].split("r")[-1])
        + ".fits"
    )
    ...
    if foundIn == -1:
        ...
        f.write(
            InputList["ID"][j]
            + " "
            + str(InputList["Distance"][j])
            + " "
            + str(InputList["theta"][j])
            + " "
            + str(InputList["A90"][j])
            + " "
            + str(luminosity)
            + " "
            + str(len(InputObservationList))
            + " "
            + str(allPossiblePoint)
            + " "
            + str(foundIn)
            + " "
            + str(0)
...
def ProduceSummaryFile(
    Source, SuggestedPointings, totalPoswindow, ID, obspar, typeSimu, datasetDir, outDir
):
    ...
    if "True" in SuggestedPointings["ObsInfo"] and Found == True:
        ...
        ascii.write(
            SuggestedPointingsC, pointingsFileC, overwrite=True, fast_writer=False
        )
        ...
        FillSummary(
            outfilename,
            ID,
            len(SuggestedPointingsC),
            totalPoswindow,
            FoundFirst,
            nP,
            np.sum(SuggestedPointings["PGW"]),
            str(2),
        )
    ...
    if "True" not in SuggestedPointings["ObsInfo"]:
        ...
        FillSummary(
            outfilename, ID, 0, totalPoswindow, foundFirst, nP, totalPGW, str(0)
        )


def ProducePandasSummaryFile(
    Source,
    SuggestedPointings,
    totalPoswindow,
    ID,
...
def ReadSummaryFile(summaryFile):
    ...
    nP, Found = np.genfromtxt(
        summaryFile, usecols=(8, 9), skip_header=1, unpack=True, dtype="str"
    )
    return nP, Found
...
def ZenithAngleCut_TwoTimes(
    prob, nside, time, time1, minProbcut, maxZenith, observatory
):
    """
...
def ComputeProbability2D_SelectClusters(
    prob,
    highres,
    radecs,
    conf,
    ...
    cat_pix = Table(
        [ipix, pix_ra, pix_dec, dp_Pix_Fov, zenith_ini, zenith_end, exposure],
        names=(
            "PIX",
            "PIXRA",
            "PIXDEC",
    ...
    grbSensPath = (
        "/grbsens_output_v3_Sep_2022/"
        + conf
        + "_configuration_EBL/grbsens-5.0sigma_t1s-t16384s_irf-"
    )
    ...
    if np.any(sortcat["ZENITH_INI"] > 55):
        ...
        if grb_result["obs_time"] == -1:
            ...
            frame = co.AltAz(
                obstime=time + datetime.timedelta(seconds=texp60),
                location=obspar.location,
            )
            catCoord60 = co.SkyCoord(
                sortcat["PIXRA"][sortcat["ZENITH_INI"] > 55],
                sortcat["PIXDEC"][sortcat["ZENITH_INI"] > 55],
                frame="fk5",
                unit=(u.deg, u.deg),
            )
    ...
    if sortcat["ZENITH_INI"][mask1 & mask2].any():
        ...
        if grb_result["obs_time"] == -1:
            ObsCase = "TimeNotEnough"
            sortcat["EXPOSURE"][
                (30 <= sortcat["ZENITH_INI"]) & (sortcat["ZENITH_INI"] <= 55)
            ] = False
        else:
            ...
            # print(Cat40)
            frame = co.AltAz(
                obstime=time + datetime.timedelta(seconds=texp40),
                location=obspar.location,
            )
            catCoord40 = co.SkyCoord(
                sortcat["PIXRA"][
                    (30 <= sortcat["ZENITH_INI"]) & (sortcat["ZENITH_INI"] <= 55)
                ],
                sortcat["PIXDEC"][
    ...
    if np.any(sortcat["ZENITH_INI"] < 30):
        ...
        if grb_result["obs_time"] == -1:
            ...
            frame = co.AltAz(
                obstime=time + datetime.timedelta(seconds=texp20),
                location=obspar.location,
            )
            catCoord20 = co.SkyCoord(
                sortcat["PIXRA"][sortcat["ZENITH_INI"] < 30],
                sortcat["PIXDEC"][sortcat["ZENITH_INI"] < 30],
                frame="fk5",
                unit=(u.deg, u.deg),
            )
